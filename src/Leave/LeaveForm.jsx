    ...
    useEffect(() => {
        ...
        const fetchRemainingLeaveDays = async (employeeId, leaveType, leaveStartDate, leaveEndDate) => {
            ...
            try {
                const token = localStorage.getItem('token')
                console.log(token)
                const response = await axios.get('http://4.236.184.244:8086/api/leaves/remaining-leaves', {
                    method:'GET',
                    headers:{
                      'Authorization' : `Bearer ${token}`,
                      'Content-Type' : 'application/json'
...
requiredFields.forEach(field => {
    console.log(`${field}: ${formData[field]}`);
        ...
        try {
            ...
            const url = isEditing
                ? `http://4.236.184.244:8086/api/leaves/update/${formData.id}`
                : `http://4.236.184.244:8086/api/leaves/submit`;
