...
export default function LeaveEmployee() {
  ...
  if (!employeeId) {
    ...
    return;
      ...
      try {
        ...
        console.log(token)
        const response = await axios.get(`http://4.236.184.244:8086/api/leaves/employee/${employeeId}`, {
          headers:{
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token')
      await axios.delete(`http://4.236.184.244:8086/api/leaves/delete/${deleteRequestId}`, {
        method:'GET',
        headers:{
          'Authorization' : `Bearer ${token}`,
          'Content-Type' : 'application/json'
